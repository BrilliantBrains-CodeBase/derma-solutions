 ...
 *
 * The same goes for the authored second block in ./schema-extra/.
 *
...
const devSchemas: Record<string, string> = import.meta.env.DEV
  ...
  : {}
const devExtraSchemas: Record<string, string> = import.meta.env.DEV
  ? (import.meta.glob('./schema-extra/*.json', { eager: true, query: '?raw', import: 'default' }) as Record<string, string>)
  : {}
...
export function JsonLd({ schemaKey }: { schemaKey: string }) {
  ...
  if (!raw) return null
  const extra = devExtraSchemas[`./schema-extra/${schemaKey}.json`]
  return (
    <Head>
      <script type="application/ld+json">{raw}</script>
      {extra && <script type="application/ld+json">{extra}</script>}
    </Head>
  )
}
